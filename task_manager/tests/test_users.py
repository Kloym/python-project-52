...
class TestLogin(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='Pop', email='john@mail.ru', password='123')
...
class TestRegistration(TestCase):
    def test_registration(self):
        responce = self.client.post(reverse('registration'), {
            'username': 'NewUser',
...
class TestUserUpdate(TestCase):
    ...
    def test_update_user(self):
        responce = self.client.post(reverse('update_user', args=[self.user.id]), {
            'username': 'UpdateUser',
